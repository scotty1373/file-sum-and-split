...
int main()
{
    ...
    int file_len  = 0;
    int size_file = 0;
    unsigned int crc_prev = 0xFFFFFFFF;
    unsigned int crc;
    fp = fopen("data_block0","rb+");
    if(fp == NULL)
    {
        printf("fopen error\n");
        return 1;
    ...
    //printf("file size : %d\n",file_len);
    for(size_file = 0; size_file<63; size_file++)
    {
        ...
        fread(buffer, 128*1024, 1, fp);
        crc = dsyslib_crc32_calc(buffer, 128*1024, crc_prev);
        crc_prev = crc;
        free(buffer);
        buffer = NULL;
    }   
