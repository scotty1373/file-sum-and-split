...
int filestat_detect_integrate()
{
    ...
    do
    {
        ...
        if(n == 0)
        {
            crc_prev = crc_init;
            fseek(tp, 0, SEEK_SET);
            crc_void_buff = (char *)malloc(sizeof(char)*4096);
            fread(crc_void_buff, 4096, 1, tp);
            p_crc = crc_void_buff;
            crc = p_crc;
            fseek(tp, offset_crc, SEEK_SET);
            for(count=0; count<63; count++)
            {
                buff_crc = (char *)malloc(sizeof(char)*128*1024);
				if(buff_crc == NULL)
				{
					printf("block_crc buffer malloc error\n");
					return 1;
				}
                fread(buff_crc, 128*1024, 1, tp);
                crc_prev = dsyslib_crc32_calc(buff_crc, 128*1024, crc_prev);
                free(buff_crc);
                buff_crc = NULL;
            }
            printf("origin: %02X\n", crc->block_crc[n]);
            printf("now: %02X\n", crc_prev);
            if(crc->block_crc[n] == crc_prev)
            {
                printf("crc checked\n");
            }
            else
...
int main()
{
    ...
    buff_temp = (char *)malloc(sizeof(char)*4096);
    if(buff_temp == NULL)
    {
        printf("malloc error\n");
        return 1;
    ...
    for(j=0; j<6; j++)
    {
        ...
        crc_compare = caculate_file_crc(header->tag[j].name, crc_init);
        if(crc_compare == header->tag[j].file_crc)
        {
            printf("CRC checked\n");
        }
        else
        {
            printf("CRC cant check\n");
        }  
        //printf("integrated file CRC: %02X\n", crc_compare);
    }
    free(buff_temp);
    fclose(fp_source);
    return 0;
